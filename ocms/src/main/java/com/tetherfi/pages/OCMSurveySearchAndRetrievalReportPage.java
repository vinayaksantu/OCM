...
public class OCMSurveySearchAndRetrievalReportPage extends BasePage  {

	public OCMSurveySearchAndRetrievalReportPage(WebDriver driver) {
		super(driver);
	}
	...
	@FindBy(xpath="//a[text()='Survey Intent']")
	private WebElement survInt;

	@FindBy(xpath="//p[@class='k-reset']")
	private WebElement groupby;

	@FindBy(xpath="//tbody/tr/td/p[@class='k-reset']/../../following-sibling::tr/td[5]")
	private WebElement groupbysurvInt;
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMCampaignSurveySearchAndRetrievalReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMCampaignSurveySearchAndRetrievalReport");
		return Status;
	}
	...
	public boolean verifySearchIsEqualTo(String details) throws Exception {
		...
		selectDropdownFromVisibleText(searchColListBox,"Respondent Name");  
		waitForJqueryLoad(driver);
		...
		{   	
			if(map1.get("Respondent Name").equals(details))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchIsNotEqualTo(String details) throws Exception {
		Boolean Status=false;
		Map<String, String> map=new HashMap<String,String>() ;
		map.put("Respondent Name", details);
		selectWebElement(searchBtn);	
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Respondent Name");  
		waitForJqueryLoad(driver);
	...
	public boolean verifySearchByTextbox(ReportDetails details) throws Exception{	
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Respondent Name").equals(details.getSearchStr()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Respondent Name").toUpperCase().contains(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchDoesNotContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(!map1.get("Respondent Name").toLowerCase().contains(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchStartsWith(String description) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown); 
		selectDropdownFromVisibleText(searchColListBox,"Respondent Phone Number");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Respondent Phone Number").toLowerCase().startsWith(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Respondent Phone Number").toUpperCase().endsWith(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifyAdvanceSearchIsEqualTo(ReportDetails reportDetails) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			System.out.println(map1.get("Respondent Name"));
			if(map1.get("Respondent Name").equalsIgnoreCase(reportDetails.getSearchStr()))
				Status= true;
			else 
				Status =false;
		}
	...
	public Boolean advancedSearchANDCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Respondent Name");
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Respondent Phone Number");
		Thread.sleep(2000);
	...
	public boolean groupby() {
		DragandDrop(survInt,droptarget);
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		if(groupby.getText().split(": ")[1].equals(groupbysurvInt.getText()))
		{return true;}
		else
			return false;		
	}
