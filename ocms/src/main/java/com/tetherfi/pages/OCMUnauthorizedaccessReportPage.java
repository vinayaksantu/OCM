...
public class OCMUnauthorizedaccessReportPage extends BasePage  {

	public OCMUnauthorizedaccessReportPage(WebDriver driver) {
		super(driver);
	}
	...
	@FindBy(xpath="//a[text()='Workstation']")
	...
	private WebElement groupbyworkstation;
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMUnauthorizedAccessReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMUnauthorizedAccessReport");
		return Status;
	}
	...
	public boolean verifySearchIsEqualTo(String details) throws Exception {
		...
		selectDropdownFromVisibleText(searchColListBox,"User ID");  
		waitForJqueryLoad(driver);
		...
		{   	
			if(map1.get("User ID").equals(details))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchByTextbox(ReportDetails details) throws Exception{	
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("User ID").equals(details.getSearchStr()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("User ID").toUpperCase().contains(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchDoesNotContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(!map1.get("User ID").toLowerCase().contains(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchStartsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("User ID").toLowerCase().startsWith(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("User ID").toUpperCase().endsWith(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public Boolean advancedSearchANDCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"User ID");
		Thread.sleep(2000);
		selectWebElement(searchCriteriaDropdownAdvSrch);
		selectDropdownFromVisibleText(searchCriteriaListboxAdvSrch,"Starts With");
		enterValueToTxtField(searchTextBoxAdvSrch,details.getSearchStr().toLowerCase());
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Workstation");
		Thread.sleep(2000);
		...
		List<Map<String,String>>UI=getDataTable();
		for(Map<String,String> map1:UI) {
			if(map1.get("User ID").toLowerCase().startsWith(details.getSearchStr().toLowerCase()) && map1.get("Workstation").contains(details.getSearchStr1())) {
			Status=true;
		}
		}
		return Status;	
	...
	public Boolean advancedSearchORCriteria(ReportDetails details) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			if(map1.get("User ID").contains(details.getSearchStr())||map1.get("Workstation").startsWith(details.getSearchStr2()))
				Status=true;
		}
		return Status;	
	}
	...
	public boolean groupby() {
		DragandDrop(workstation,droptarget);
		try {
			Thread.sleep(2000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		if(groupby.getText().split(": ")[1].equals(groupbyworkstation.getText()))
		{return true;}
		else
			return false;		
	}
