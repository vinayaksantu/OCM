...
public class OCMSMSHostTransactionReportPage extends BasePage  {

	public OCMSMSHostTransactionReportPage(WebDriver driver) {
		super(driver);}
	...
	@FindBy(xpath="//a[text()='Request Date Time']")
	private WebElement reqDT;

	@FindBy(xpath="//p[@class='k-reset']")
	private WebElement groupby;

	@FindBy(xpath="//tbody/tr/td/p[@class='k-reset']/../../following-sibling::tr/td[3]")
	private WebElement groupbyreqDT;
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMSMSHostTransactionReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMSMSHostTransactionReport");
		return Status;
	}
	...
	public boolean verifyAdvanceSearch(ReportDetails reportDetails) throws Exception {
		...
		{
			System.out.println(map1.get("Response Message"));
			if(map1.get("Response Message").equalsIgnoreCase(reportDetails.getSearchStr()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifySearchIsNotEqualTo(String details) throws Exception {
		Boolean Status=false;
		Map<String, String> map=new HashMap<String,String>() ;
		map.put("Session ID", details);
		selectWebElement(searchBtn);	
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Session ID");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
	...
	public boolean verifySearchIsEqualTo(String details) throws Exception {
		...
		selectWebElement(searchBtn);	
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Response Message");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Response Message").equals(details))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchContains(String details) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Transaction Name");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Transaction Name").toUpperCase().contains(details.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchDoesNotContains(String description) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Request Message");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(!map1.get("Request Message").toLowerCase().contains(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchStartsWith(String details) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Response Message").toLowerCase().startsWith(details.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String details) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Response Message").toUpperCase().endsWith(details.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public Boolean advancedSearchANDCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Transaction Name");
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Response Message");
		Thread.sleep(2000);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Transaction Name").contains(details.getSearchStr()) && map1.get("Response Message").startsWith(details.getSearchStr1()))
				Status= true;
			else 
				Status= false;
		}
	...
	public Boolean advancedSearchORCriteria(ReportDetails details) throws Exception {
		...
		selectWebElement(searchColDropdownAdvSrchReportPage);
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Response Message");
		Thread.sleep(2000);
		selectWebElement(searchCriteriaDropdownAdvSrch);
		...
		selectWebElement(searchColDropdownAdvSrchReportPage1);
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Transaction Name");
		Thread.sleep(2000);
		selectWebElement(searchCriteriaDropdownAdvSrch1);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Response Message").equals(details.getSearchStr()) || map1.get("Transaction Name").toLowerCase().endsWith(details.getSearchStr1().toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean groupby() throws Exception {
		Thread.sleep(1000);
		DragandDrop(reqDT,droptarget);
		try {
			Thread.sleep(3000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		if(groupby.getText().split(": ")[1].equals(groupbyreqDT.getText()))
		{return true;}
		else
			return false;		
	}	

	public boolean verifySearchAfter(String details) throws Exception {
		Boolean Status=false;
		Map<String, String> map=new HashMap<String,String>() ;
		map.put("Request Date Time", details);
		selectWebElement(searchBtn);	
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Request Date Time");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		selectDropdownFromVisibleText(searchCriteriaListbox,"Is after");		   
		waitForJqueryLoad(driver); 
		Thread.sleep(2000);
		...
		if((rowdata.getText().compareTo(details) > 0 ))		
			Status= true;
		else 
			Status= false;
		}
	...
	public boolean verifySearchBefore(String details) throws Exception {
		Boolean Status=false;
		Map<String, String> map=new HashMap<String,String>() ;
		map.put("Request Date Time", details);
		selectWebElement(searchBtn);	
		...
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		selectDropdownFromVisibleText(searchCriteriaListbox,"Is before");		   
		waitForJqueryLoad(driver); 
		Thread.sleep(2000);
		System.out.println(details);
		selectWebElement(searchTextBox1);
		...
		for (Map<String,String> map1: UI){   
			System.out.println(map1);	
			System.out.println(rowdata.getText().compareTo(details) < 0 );
		if((rowdata.getText().compareTo(details) < 0 ))		
//			System.out.println(rowdata.getText().compareTo(details) < 0 );
			Status= true;
		else 
	...
	public void sortAscByResponseDate() {
		selectWebElement(headersDropdown.get(2));
		waitForJqueryLoad(driver);
