...
public class OCMSMSInteractionReportPage extends BasePage  {

	public OCMSMSInteractionReportPage(WebDriver driver) {
		super(driver);}
	...
	private WebElement groupbysource;
	...
	public boolean verifySorting() throws Exception {
		...
		for (int i=0; i<= pages; i++) {
			...
			for(int j=4;j<5;j++){
				...
				selectWebElement(sortAscending.get(k-4));
				waitForJqueryLoad(driver);
				...
				selectWebElement(headersDropdown.get(k));
				waitForJqueryLoad(driver);
				selectWebElement(sortDescending.get(k-4));
				waitForJqueryLoad(driver);
				k++;
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMSMSInteractionReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMSMSInteractionReport");
		return Status;
	}
	...
	public boolean verifyAdvanceSearch(ReportDetails reportDetails) throws Exception {
		...
		{
			System.out.println(map1.get("Customer Number"));
			if(map1.get("Customer Number").equalsIgnoreCase(reportDetails.getSearchStr()))
				Status= true;
			else 
				Status =false;
		}
	...
	public void searchwithoutextsearch(ReportDetails details) throws Exception {
		...
		selectWebElement(searchCloseBtn.get(4));		
	}
	...
	public boolean verifySearchContains(String details) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Customer Number");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Customer Number").toUpperCase().contains(details.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchDoesNotContains(String details) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Source");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(!map1.get("Source").toLowerCase().contains(details.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchStartsWith(String details) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Source").toLowerCase().startsWith(details.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String details) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Source").toUpperCase().endsWith(details.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchByTextbox(ReportDetails details) throws Exception{	
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Customer Number").equals(details.getSearchStr()))
				Status= true;
			else 
				Status= false;
		}
	...
	public Boolean advancedSearchANDCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Customer Number");
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Source");
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		if(rowdata2.getText().equals(details.getSearchStr()) && rowdata1.getText().equals(details.getSearchStr1())) {
			Status=true;
		}
		return Status;	
	}
	...
	public Boolean advancedSearchORCriteria(ReportDetails details) throws Exception {
		...
		{
			if(rowdata2.getText().equals(details.getSearchStr())||rowdata1.getText().equals(details.getSearchStr2()))
				Status=true;
		}
		return Status;	
	}
	...
	public boolean groupby() {
		...
		if(groupby.getText().split(": ")[1].equals(groupbysource.getText()))
		{return true;}
		else
			return false;		
	}	
	...
	public void sortAscByStartDate() {
