...
public class OCMAnalysisCountReportPage extends BasePage  {

	public OCMAnalysisCountReportPage(WebDriver driver) {
		super(driver);
	}
	...
	@FindBy(xpath="//a[text()='Agent Transfers]")
	private WebElement agtTrans;
...
//	}*/
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMIvrMenuAnalysisReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMIvrMenuAnalysisReport");
		return Status;
	}
	...
	public boolean verifySearchIsEqualTo(String details) throws Exception {
		...
		selectDropdownFromVisibleText(searchColListBox,"Menu");  
		waitForJqueryLoad(driver);
		...
		{   	
			if(map1.get("Menu").equals(details))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchByTextbox(ReportDetails details) throws Exception{	
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Menu").equals(details.getSearchStr()))	
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Menu").toUpperCase().contains(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchDoesNotContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(!map1.get("Menu").toLowerCase().contains(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchStartsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Menu").toLowerCase().startsWith(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Menu").toUpperCase().endsWith(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean advancedSearchORCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Menu");
		Thread.sleep(2000);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Menu").equals(details.getSearchStr()) && map1.get("AccessCount").endsWith(details.getSearchStr1()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifyViewSummary() throws Exception {		
		...
		waitForJqueryLoad(driver);	
		Thread.sleep(2000);
		selectWebElement(summarypopup);					 
		if(summaryPopupLabel.isDisplayed())				
			status=true;			
		else
			status=false;		
		return status;
