...
public class OCMChatInteractionReportPage extends BasePage  {
	...
	public OCMChatInteractionReportPage(WebDriver driver) {
		super(driver);
	}
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMChatInteractionReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMChatInteractionReport");
		return Status;
	}
	...
	public Boolean advancedSearchANDCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Chat End Reason");
		Thread.sleep(2000);
