...
public class OCMLineOverallStatisticReportPage extends BasePage  {

	public OCMLineOverallStatisticReportPage(WebDriver driver) {
		super(driver);
	}
	...
	@FindBy(xpath="//a[text()='Register/Deregister']")
	private WebElement regDereg;

	@FindBy(xpath="//p[@class='k-reset']")
	private WebElement groupby;

	@FindBy(xpath="//tbody/tr/td/p[@class='k-reset']/../../following-sibling::tr/td[4]")
	private WebElement groupbyregDereg;
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMLINEOverallStatisticReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMLINEOverallStatisticReport");
		return Status;
	}
	...
	public boolean verifyAdvanceSearch(ReportDetails reportDetails) throws Exception {
		...
		{
			System.out.println(map1.get("CIF"));
			if(map1.get("CIF").equalsIgnoreCase(reportDetails.getSearchStr()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifySearchIsNotEqualTo(String details) throws Exception {
		...
		selectWebElement(searchBtn);	
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"CIF");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
	...
	public boolean verifySearchEqualTo(String details) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("CIF").equals(details))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchContains(String details) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"MID");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("MID").toUpperCase().contains(details.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchDoesNotContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(!map1.get("MID").toLowerCase().contains(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchStartsWith(String description) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown); 
		selectDropdownFromVisibleText(searchColListBox,"Register/Deregister");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Register/Deregister").toLowerCase().startsWith(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("CIF").toUpperCase().endsWith(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchByTextbox(ReportDetails details) throws Exception{	
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("CIF").equals(details.getSearchStr()))
				Status= true;
			else 
				Status= false;
		}
	...
	public Boolean advancedSearchANDCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"CIF");
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Register/Deregister");
		Thread.sleep(2000);
	...
	public boolean groupby() throws Exception {
		Thread.sleep(1000);
		DragandDrop(regDereg,droptarget);
		try {
			Thread.sleep(5000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		if(groupby.getText().split(": ")[1].equals(groupbyregDereg.getText()))
		{return true;}
		else
			return false;		
	}	
