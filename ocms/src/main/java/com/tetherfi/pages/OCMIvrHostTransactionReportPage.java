...
public class OCMIvrHostTransactionReportPage extends BasePage  {

	public OCMIvrHostTransactionReportPage(WebDriver driver) {
		super(driver);
	}
	...
	@FindBy(xpath="//a[text()='Request Message']")
	...
	//export to excel in IVRHostTansation
	...
	private WebElement groupbyRequestMessage;
	...
	public void SortByAscending() {
		selectWebElement(RequestMessage);
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMIvrHostTransactionReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
	...
	public boolean verifySearchStartsWith(String details) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("User ID").toLowerCase().startsWith(details.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String details) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("User ID").toUpperCase().endsWith(details.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean groupby() {
		...
		if(groupby.getText().split(": ")[1].equals(groupbyRequestMessage.getText()))
		{return true;}
		else
			return false;		
	}
