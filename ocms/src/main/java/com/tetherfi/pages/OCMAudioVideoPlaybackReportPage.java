...
public class OCMAudioVideoPlaybackReportPage extends BasePage  {
	...
	public void SortByAscending() {
		selectWebElement(supervisorname);
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMAudioVideoPlaybackReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMAudioVideoPlaybackReport");
		return Status;
	}
	...
	public boolean verifySearchIsEqualTo(String details) throws Exception {
		...
		{   	
			if(map1.get("Skill").equals(details))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchIsNotEqualTo(String details) throws Exception {
		Boolean Status=false;
		Map<String, String> map=new HashMap<String,String>() ;
		map.put("Chat End Reason", details);
		selectWebElement(searchBtn);	
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Chat End Reason");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
	...
	public boolean verifyAdvanceSearchIsNotEqualTo(ReportDetails reportDetails) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			...
			if(map1.get("Channel").equalsIgnoreCase(reportDetails.getSearchStr()))
				Status= false;
			else 
				Status =true;
		}
	...
	public boolean verifyAdvanceSearchDoesNotContains(ReportDetails reportDetails) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			System.out.println(map1.get("Skill"));
			if(!map1.get("Skill").toUpperCase().contains(reportDetails.getSearchStr()))				
				Status= true;
			else 
				Status =false;
		}
