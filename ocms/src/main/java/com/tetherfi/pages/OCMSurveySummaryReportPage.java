...
public class OCMSurveySummaryReportPage extends BasePage  {

	public OCMSurveySummaryReportPage(WebDriver driver) {
		super(driver);}
	...
	//export to excel in ChatInteractionPage
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMSurveySummaryReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMSurveySummaryReport");
		return Status;
	}
	...
	public boolean verifyAdvanceSearchIsEqualTo(ReportDetails reportDetails) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			System.out.println(map1.get("Survey ID"));
			if(map1.get("Survey ID").equalsIgnoreCase(reportDetails.getSearchStr()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifyAdvanceSearchIsNotEqualTo(ReportDetails reportDetails) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			System.out.println(map1.get("Chat End Reason"));
	...
	public Boolean advancedSearchANDCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Survey ID");
		Thread.sleep(2000);
...
//		System.out.println("Printing UI Results"+" "+UI);	
	...
	private List<Map<String, String>> getDataTable2() {
		int item=Integer.valueOf(items.getText().split("of ")[1].split(" items")[0]);
		int pagersize=Integer.valueOf(pagerSize.getText());
		int pages=(item%pagersize==0)?item/pagersize-1:item/pagersize;
		List<Map<String,String>> arr=new ArrayList<Map<String,String>>();
