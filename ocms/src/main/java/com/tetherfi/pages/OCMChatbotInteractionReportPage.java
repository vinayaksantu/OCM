...
public class OCMChatbotInteractionReportPage extends BasePage  {
	
	  public OCMChatbotInteractionReportPage(WebDriver driver) {
	        super(driver);
	    }
...
public boolean verifyExportToExcel(String filePath) {
	...
	for (final File f : folder.listFiles()) {
	    if (f.getName().startsWith("OCMChatbotInteractionReport")) {
        f.delete();
    }
}
selectWebElement(exporttoexcel);
...
Boolean Status=verifyExportPageFileDownload(filePath, "OCMChatbotInteractionReport");
	return Status;
}
