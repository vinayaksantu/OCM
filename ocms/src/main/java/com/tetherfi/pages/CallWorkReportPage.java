...
public class CallWorkReportPage extends BasePage  {
	...
	private WebElement CloseDrillGridOne1;

	@FindBy(xpath="//div[@class='modal-dialog modal-lg animated bounceInRight']//span[@aria-hidden='true'][contains(normalize-space(),'×')]")
	...
	private WebElement pagerSizeDrillGridOne1;
	
	@FindBy(xpath="//div[@id='drillGrid']//span[@class='k-input']")
	...
	@FindBy(xpath="(//div[@id='gridDrillOne']//span[@class='k-icon k-i-arrow-60-right'])")
	private WebElement nextPageIconDrillOne2;
	
	@FindBy(xpath="//div[@id='drillGrid']//span[@class='k-icon k-i-arrow-60-right']")
	private WebElement nextPageIconDrillOne;
	
	@FindBy(xpath="//div[@id='drillGrid']//span[@class='k-pager-info k-label']")
	...
	private WebElement groupbyAgentname;
	...
	@FindBy(xpath="//div[@id='popup']//button[2]")
	private WebElement DrillRefreshBtn;
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("Workcode Report")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath,"Workcode Report");
		return Status;
	}
	...
	public boolean verifySearchByTextbox(ReportDetails details) throws Exception{	
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("WorkcodeDesc").equals(details.getSearchStr()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchContains(String details) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Avaya Login Id");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Avaya Login Id").contains(details))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchStartsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Avaya Login Id").toLowerCase().startsWith(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Avaya Login Id").toUpperCase().endsWith(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
...
//		waitUntilWebElementIsVisible(rows.get(0));
		Thread.sleep(2000);
		selectWebElement(rows.get(0));
	...
	public List<String> getWorkCodes() throws Exception {
		int item=Integer.valueOf(items.getText().split("of ")[1].split(" items")[0]);
		int pagersize=Integer.valueOf(pagerSize.getText());
		int pages=(item%pagersize==0)?item/pagersize-1:item/pagersize;
		...
		for(int k=0;k<=pages;k++){
			...
			for(int i=1;i<rows.size();i++) {
				...
				String col=null;
				for(int j=0;j<headers.size();j++){
					if(headers.get(j).getText().equals("Workcode")){
						col=cols.get(j).getText();
						workCodes.add(col);
					}
				}
	...
	public List<String> getAgents() throws Exception {
		...
		for(int k=0;k<=pages;k++){
			...
			for(int i=1;i<rows.size();i++) {
				...
				String col=null;
				for(int j=0;j<headers.size();j++){
					if(headers.get(j).getText().equals("Avaya Login Id")){
						col=cols.get(j).getText();
						agents.add(col);
					}
				}
	...
	public void clickOnWorkCodeRowOnMainReport(int rowNo) throws InterruptedException {
		Thread.sleep(2000);
	...
	public boolean verifyDatabaseDrillGridOne(String queryDrillGridOne,ReportDetails details, String WorkCode, String AgentId) throws Exception {
		//get dates from xl - step 2
		String reportbeforedate = details.getStartDate();
		String reportafterdate=details.getEndDate();
		//change date formats - step 3
		...
		queryDrillGridOne=queryDrillGridOne.replaceAll("WorkCodeCapturedFromUI", WorkCode);
	...
	public boolean groupby() {
		...
		if(groupby.getText().split(": ")[1].equals(groupbyAgentname.getText()))
		{return true;}
		else
			return false;		
	}
	...
	public boolean advancedSearchORCriteria(ReportDetails details) throws Exception {
		...
		selectWebElement(searchColDropdownAdvSrchReportPage1);
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Avaya Login Id");
		Thread.sleep(2000);
		selectWebElement(searchCriteriaDropdownAdvSrch1);
		...
		{
			if(rowdata.getText().equals(details.getSearchStr())||rowdata1.getText().contains(details.getSearchStr2()))
				Status=true;
		}
		return Status;	
	}
	
	public void refreshDrillDownData() {
		selectWebElement(DrillRefreshBtn);
	}
