...
public class OCMChatHostTransactionReportPage extends BasePage  {
	
	  public OCMChatHostTransactionReportPage(WebDriver driver) {
	        super(driver);
	    }
...
public boolean verifyExportToExcel(String filePath) {
	...
	for (final File f : folder.listFiles()) {
	    if (f.getName().startsWith("OCM Chat Host Transaction Report")) {
        f.delete();
    }
}
selectWebElement(exporttoexcel);
...
Boolean Status=verifyExportPageFileDownload(filePath, "OCM Chat Host Transaction Report");
	return Status;
}
