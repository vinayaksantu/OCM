...
public class OCMCallTransferReportPage extends BasePage  {

	public OCMCallTransferReportPage(WebDriver driver) {
		super(driver);
	}
	...
	@FindBy(xpath="//h2[text()='Call Transfer Report Summary Data ']")
	private WebElement summaryPopupLabel1;

	@FindBy(xpath="//tbody/tr/td/table[@id='countreportvaluesTable']/tr[1]/td[1]")
	private WebElement summaryPopupLabel;

	@FindBy(xpath="//SPAN[@aria-hidden='true'][text()='×']")
	private List<WebElement> ClosePopUp;

	@FindBy(xpath="//table/tbody/tr/td")
	private List<WebElement> rows;

	@FindBy(xpath="//div[@id='summaryPopUp']//span[@aria-hidden='true'][normalize-space()='×']")
	private WebElement summaryPopUpClose;
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMCallTransferReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMCallTransferReport");
		return Status;
	}
	...
	public boolean verifySearchContains(String description) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Last Menu");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Last Menu").toUpperCase().contains(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchDoesNotContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(!map1.get("Last Menu").toLowerCase().contains(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Last Menu").toUpperCase().endsWith(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifyViewSummary() throws Exception {		
		boolean status=false;
		selectWebElement(ViewSummaryButton);
		waitForJqueryLoad(driver);
		waitUntilWebElementIsVisible(summaryPopUpClose);
		if(summaryPopUpClose.isDisplayed()) 
			status=true;
		else 
			status=false;
		selectWebElement(summaryPopUpClose);
		return status;
	}
	...
	private List<Map<String, String>> getDataTable1() throws InterruptedException {
		...
		for(int k=0;k<=pages;k++){
			...
			if(k!=pages)
			{
				waitUntilWebElementIsClickable(nextPageIcon);
				nextPageIcon.click();
				waitForJqueryLoad(driver);}
		}
		return arr;
