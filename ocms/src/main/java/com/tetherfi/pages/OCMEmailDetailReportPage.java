...
public class OCMEmailDetailReportPage extends BasePage{

	public OCMEmailDetailReportPage(WebDriver driver) {
		super(driver);}
	...
	@FindBy(xpath="//a[text()='Mail Status']")
	...
	private WebElement groupbyEmailStatus;
	...
	public boolean verifySearchByTextbox(ReportDetails details) throws Exception{	
		...
		{   	
			if(map1.get("Email ID").equals(details.getSearchStr()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchContains(String details) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Mail Status");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Mail Status").toUpperCase().contains(details.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchDoesNotContains(String details) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(!map1.get("Mail Status").toLowerCase().contains(details.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchStartsWith(String details) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Session ID").toLowerCase().startsWith(details.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String details) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Session ID").toLowerCase().endsWith(details.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public void searchwithoutextsearch(ReportDetails details) {
		selectWebElement(searchBtn);		
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Subject");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
	...
	public boolean verifyAdvancedSearchIsEqualTo(ReportDetails reportDetails) throws Exception {
		...
		List<Map<String,String>>UI=getDataTable();
		for(Map<String,String> map1:UI){
			System.out.println(map1.get("Email ID"));
			System.out.println("Parameter used for search is"+" "+reportDetails.getSearchStr());
			if(map1.get("Email ID").equalsIgnoreCase(reportDetails.getSearchStr()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifyAdvancedSearchContains(ReportDetails details) throws Exception {
		...
		List<Map<String,String>>UI=getDataTable();
		for(Map<String,String> map1:UI){
			if(map1.get("Subject").contains(details.getSearchStr()))	
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifyAdvancedSearchStartsWith(ReportDetails details) throws Exception {
		...
		for(Map<String,String> map1:UI){
			...
			if(map1.get("Session ID").startsWith(details.getSearchStr()))				
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifyAdvancedSearchEndsWith(ReportDetails details) throws Exception {
		...
		for(Map<String,String> map1:UI){
			System.out.println(map1.get("Session ID"));
			if(map1.get("Session ID").endsWith(details.getSearchStr()))				
				Status= true;
			else 
				Status =false;
		}
	...
	public Boolean advancedSearchORCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Email ID");
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Session ID");
		Thread.sleep(2000);
		...
		List<Map<String,String>> UI=getDataTable(); 
		for (Map<String,String> map1: UI){   	
			if(map1.get("Email ID").equalsIgnoreCase(details.getSearchStr()) || map1.get("Session ID").equalsIgnoreCase(details.getSearchStr1()))
				Status= true;
			else 
				Status= false;
		}
	...
	public Boolean advancedSearchANDCriteria(ReportDetails details) throws Exception {
		...
		selectWebElement(searchColDropdownAdvSrchReportPage1);
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Mail Status");
		Thread.sleep(2000);
		selectWebElement(searchCriteriaDropdownAdvSrch1);
		selectDropdownFromVisibleText(searchCriteriaListboxAdvSrch1,"Is equal to");
		enterValueToTxtField(searchTextBoxAdvSrch1,details.getSearchStr1());
		...
		List<Map<String,String>> UI=getDataTable(); 
		for (Map<String,String> map1: UI){   	
			if(map1.get("Email ID").equalsIgnoreCase(details.getSearchStr()) && map1.get("Mail Status").equalsIgnoreCase(details.getSearchStr1()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean groupby() {
		DragandDrop(mailStatus,droptarget);
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		if(groupby.getText().split(": ")[1].equals(groupbyEmailStatus.getText()))
		{return true;}
		else
			return false;		
	}
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("EmailDetailReport")) {
				f.delete();
			}
		}
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "EmailDetailReport");
		return Status;
	}
