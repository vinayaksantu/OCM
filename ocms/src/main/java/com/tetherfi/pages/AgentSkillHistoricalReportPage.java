...
public class AgentSkillHistoricalReportPage extends BasePage  {

	public AgentSkillHistoricalReportPage(WebDriver driver) {
		super(driver);
	}
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMSkillHistoricalReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMSkillHistoricalReport");
		return Status;
	}
	...
	public boolean verifySearchByTextbox(ReportDetails details) throws Exception{	
		...
		for (Map<String,String> map1: UI)
		{ 
			if(map1.get("Skill ID").equals(details.getSearchStr()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifyAdvanceSearch(ReportDetails reportDetails) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			System.out.println(map1.get("Skill ID"));
			if(map1.get("Skill ID").equalsIgnoreCase(reportDetails.getSearchStr()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean groupby() {
		DragandDrop(SkillId,droptarget);
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
	...
	public boolean verifyDatabaseDrillGridOne(String queryDrillGridOne,ReportDetails details, String SkillId) {
		//get dates from xl - step 2
		String reportbeforedate = details.getStartDate();
		String reportafterdate=details.getEndDate();
		//change date formats - step 3
	...
	public String captureFirstRowSkillId() {
		return FirstRowFirstCell.getText();
	}

	public void clickOnFirstRowOnMainReport() {
		FirstRowFirstCell.click();
	}
	public String captureFirstRowDate() {
		return DrillGridOneFirstCell.getText();
	}
	...
	public List<String> getSkillDates() {
		int item=Integer.valueOf(drillGridOneItems.getText().split("of ")[1].split(" items")[0]);
		int pagersize=Integer.valueOf(pagerSizeDrillGridOne.getText());
		int pages=(item%pagersize==0)?item/pagersize-1:item/pagersize;
		...
		for(int k=0;k<=pages;k++) {
			...
			for(int i=1;i<rows.size();i++) {
				List<WebElement> cols=rows.get(i).findElements(By.tagName("td"));
				skillDates.add(cols.get(0).getText());
			}
			if(k!=pages)
			{
				nextPageIconDrillOne.click();
	...
	public Boolean advancedSearchAddCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Skill ID");
		Thread.sleep(2000);
	...
	public Boolean advancedSearchORCriteria(ReportDetails details) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			if(map1.get("Skill ID").toLowerCase().equals(details.getSearchStr().toLowerCase())|| map1.get("Skill Name").toLowerCase().startsWith(details.getSearchStr2().toLowerCase()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifyAdvanceSearchContains(ReportDetails reportDetails) {
		...
		{
			System.out.println(map1.get("Skill Name"));
			if(map1.get("Skill Name").toLowerCase().contains(reportDetails.getSearchStr().toLowerCase()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifyAdvanceSearchStartsWith(ReportDetails reportDetails) {
		...
		{
			System.out.println(map1.get("Skill Name"));
			if(map1.get("Skill Name").toLowerCase().startsWith(reportDetails.getSearchStr().toLowerCase()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifyAdvanceSearchEndsWith(ReportDetails reportDetails) {
		...
		{
			System.out.println(map1.get("Skill Name"));
			if(map1.get("Skill Name").toLowerCase().endsWith(reportDetails.getSearchStr().toLowerCase()))
				Status= true;
			else 
				Status =false;
		}
