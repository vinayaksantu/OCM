...
public class NiceIntegrationReportPage extends BasePage{

	public NiceIntegrationReportPage(WebDriver driver) {
		super(driver);}
	...
	private List<WebElement> searchCloseBtn;
	...
	@FindBy(xpath="//a[text()='Intent Name']")
	...
	private WebElement groupbyintentname;
	
	@FindBy(xpath="//button[@id='push-selected-data']")
	private WebElement pushdataBtn;
	
	@FindBy(xpath="//button[@id='push-all-data']")
	private WebElement pushAllData;
	
	@FindBy(css="#popupDrill #popup")
	private WebElement DrilldownPopUp;
	
	@FindBy(xpath="//button[@class='close']")
	private List<WebElement> DrillClose;
	...
	private List<Map<String,String>> getdata(){
		...
		for(int i=1;i<rows.size();i++) {
			...
			for(int j=0;j<headers.size();j++) {
				...
				col=cols.get(j).getText();
				if(col.equals(""))
					map.put(headers.get(j).getText(),null);
				else
					map.put(headers.get(j).getText(),col);
			}
			arr.add(map);
	...
	public boolean verifyArrowMoveForPreviousAndNextPage() throws Exception{
		...
		if(!nextPageIcon.getAttribute("class").contains("k-state-disabled")){
			...
			System.out.println("previous and next page icon disabled");
			status=true;
		}
		return status;
	}
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("NiceIntegrationReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "NiceIntegrationReport");
		return Status;
	}
	...
	public boolean verifyAdvanceSearch(ReportDetails reportDetails) throws Exception {
		...
		{
			System.out.println(map1.get("Start Date Time"));
			if(map1.get("Start Date Time").equals(reportDetails.getSearchStr()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifyAdvanceSearch1(ReportDetails reportDetails) throws Exception {
		Boolean Status=false;
		waitForJqueryLoad(driver);
		Thread.sleep(3000);
		List<Map<String,String>>UI=getDataTable();
		for(Map<String,String> map1:UI)
		{
			System.out.println(map1.get("Intent Name"));
			if(map1.get("Intent Name").contains(reportDetails.getSearchStr()))
				Status= true;
			else 
				Status =false;
		}
	...
	public void searchwithoutextsearch(ReportDetails details) throws Exception {
		...
		selectWebElement(searchCloseBtn.get(0));		
	}
	...
	public boolean verifySearchIsNotEqualTo(String details) throws Exception {
		Boolean Status=false;
		Map<String, String> map=new HashMap<String,String>() ;
		map.put("End Date Time", details);
		selectWebElement(searchBtn);	
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"End Date Time");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		{   	
			...
			if(map1.get("End Date Time").equals(details))
				Status= false;
			else 
				Status= true;
		}
	...
	public boolean verifySearchContains(String details) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Intent Name");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Intent Name").toUpperCase().contains(details.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchDoesNotContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(!map1.get("Intent Name").toLowerCase().contains(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchStartsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Intent Name").toLowerCase().startsWith(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Intent Name").toUpperCase().endsWith(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public Boolean advancedSearchANDCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Start Date Time");
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"Intent Name");
		Thread.sleep(2000);
		...
		Thread.sleep(5000);*/
		if(rowdata2.getText().equals(details.getSearchStr()) && rowdata1.getText().startsWith(details.getSearchStr1())) {
			Status=true;
		}
		return Status;	
	...
	public Boolean advancedSearchORCriteria(ReportDetails details) throws Exception {
		...
		{
			if(rowdata2.getText().equals(details.getSearchStr())||rowdata1.getText().contains(details.getSearchStr2()))
				Status=true;
		}
		return Status;	
	...
	public boolean groupby() {
		DragandDrop(intentname,droptarget);
		try {
			Thread.sleep(3000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		if(groupby.getText().split(": ")[1].equals(groupbyintentname.getText()))
		{return true;}
		else
			return false;		
	}	
	...
	public boolean verifySearchAfterorEqualTo(String details) throws Exception {
		Boolean Status=false;
		Map<String, String> map=new HashMap<String,String>() ;
		map.put("Start Date Time", details);
		selectWebElement(searchBtn);	
		selectWebElement(searchColDropdown);  
		selectDropdownFromVisibleText(searchColListBox,"Start Date Time");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		if((rowdata.getText().compareTo(details) > 0 ) || (rowdata.getText().compareTo(details) == 0 ))		
			Status= true;
		else 
			Status= false;
		}
	...
	public boolean verifyPushData(ReportDetails reportDetails) throws Exception {
		boolean status=false;
		selectWebElement(pushdataBtn);
	...
	public boolean verifyPushDataWithdata(ReportDetails reportDetails) throws Exception {
		boolean status=false;
		selectWebElement(rowdata7);
		selectWebElement(pushdataBtn);
		waitUntilWebElementIsVisible(errorMsg.get(0));
		System.out.println(errorMsg.get(0).getText());
		System.out.println("×\nSelect a checkbox");
		if(errorMsg.get(0).getText().equals("Unable to send data to NICE. Data Push Failed!"))
			{System.out.println(errorMsg.get(0).getText());
			status=true;}
		else
			status=false;
	...
	public boolean verifyDrillDown(ReportDetails reportDetails) throws Exception {
		boolean status=false;
		selectWebElement(rows.get(0));
		Thread.sleep(2000);
		waitForJqueryLoad(driver);
		if(DrilldownPopUp.isDisplayed())
			status= true;
		Thread.sleep(2000);
		//selectWebElement(DrillClose.get(0));	
		return status;
	}
