...
public class OCMSystemEventReportPage extends BasePage  {

	public OCMSystemEventReportPage(WebDriver driver) {
		super(driver);
		}
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMTRSSystemEventReport")) {
				f.delete();
			}
		}
		//waitForJqueryLoad(driver); 
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMTRSSystemEventReport");
		return Status;
	}
	...
	public boolean verifySearchStartsWith(String description) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown); 
		selectDropdownFromVisibleText(searchColListBox,"Supervisor Name");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Supervisor Name").toLowerCase().startsWith(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String description) throws Exception {
		...
		selectWebElement(searchBtn);
		selectWebElement(searchColDropdown); 
		selectDropdownFromVisibleText(searchColListBox,"Action Summary");  
		waitForJqueryLoad(driver);
		selectWebElement(searchCriteriaDropdown);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Action Summary").toUpperCase().endsWith(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public Boolean advancedSearchAndCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Total User Onboarded");
		Thread.sleep(2000);
		...
		for(Map<String,String> map1:UI)
		{
			...
			if(map1.get("Total User Onboarded").toLowerCase().equals(details.getSearchStr().toLowerCase()) &&map1.get("Org Unit").toLowerCase().contains(details.getSearchStr1().toLowerCase()))
				Status= true;
			else 
				Status =false;
		}
	...
	public Boolean advancedSearchORCriteria(ReportDetails details) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			if(map1.get("Total User Onboarded").toLowerCase().equals(details.getSearchStr().toLowerCase())|| map1.get("Org Unit").toLowerCase().startsWith(details.getSearchStr2().toLowerCase()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean verifyAdvanceSearchContains(ReportDetails reportDetails) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			...
			if(map1.get("Event Name").toLowerCase().contains(reportDetails.getSearchStr().toLowerCase()))
				Status= true;
			else 
				Status =false;
		}
