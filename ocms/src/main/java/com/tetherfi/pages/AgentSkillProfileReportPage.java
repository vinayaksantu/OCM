...
public class AgentSkillProfileReportPage extends BasePage  {
	...
	@FindBy(xpath="//a[text()='Skill Name-ACD/Abandoned Calls']")
	...
	public boolean verifyExportToExcel(String filePath) {
		...
		for (final File f : folder.listFiles()) {
			if (f.getName().startsWith("OCMSkillProfileReport")) {
				f.delete();
			}
		}
		selectWebElement(exporttoexcel);
		...
		Boolean Status=verifyExportPageFileDownload(filePath, "OCMSkillProfileReport");
		return Status;
	}
	...
	public boolean verifySearchIsEqualTo(String details) throws Exception {
		...
		selectDropdownFromVisibleText(searchColListBox,"Skill Name-ACD/Abandoned Calls");  
		waitForJqueryLoad(driver);
		...
		{   	
			if(map1.get("Skill Name-ACD/Abandoned Calls").equals(details))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchByTextbox(ReportDetails details) throws Exception{	
		...
		searchbyfeatureTextBox.sendKeys(details.getSearchStr());//given an identification value
		Thread.sleep(2000);
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Skill Name-ACD/Abandoned Calls").equals(details.getSearchStr()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Skill Name-ACD/Abandoned Calls").toUpperCase().contains(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchDoesNotContains(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(!map1.get("Skill Name-ACD/Abandoned Calls").toLowerCase().contains(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchStartsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Skill Name-ACD/Abandoned Calls").toLowerCase().startsWith(description.toLowerCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifySearchEndsWith(String description) throws Exception {
		...
		for (Map<String,String> map1: UI)
		{   	
			if(map1.get("Skill Name-ACD/Abandoned Calls").toUpperCase().endsWith(description.toUpperCase()))
				Status= true;
			else 
				Status= false;
		}
	...
	public boolean verifyAdvanceSearch(ReportDetails reportDetails) throws Exception {
		...
		for(Map<String,String> map1:UI)
		{
			System.out.println(map1.get("Skill Name-ACD/Abandoned Calls"));
			if(map1.get("Skill Name-ACD/Abandoned Calls").equalsIgnoreCase(reportDetails.getSearchStr()))
				Status= true;
			else 
				Status =false;
		}
	...
	public boolean groupby() {
		DragandDrop(SkillNamecolumn,droptarget);
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
	...
	public Boolean advancedSearchANDCriteria(ReportDetails details) throws Exception {
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage,"Skill Name-ACD/Abandoned Calls");
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		selectDropdownFromVisibleText(searchColListBoxAdvSrchReportPage1,"0~5 sec");
		Thread.sleep(2000);
		...
		{
			System.out.println(map1.get("Skill Name-ACD/Abandoned Calls"));
			if(map1.get("Skill Name-ACD/Abandoned Calls").toLowerCase().equals(details.getSearchStr().toLowerCase()) &&map1.get("0~5 sec").toLowerCase().contains(details.getSearchStr1().toLowerCase()))
				Status= true;
			else 
				Status =false;
		}
